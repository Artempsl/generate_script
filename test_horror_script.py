"""Test horror script generation with new storytelling guidelines."""
import os
import time
import httpx
import asyncio

async def test_horror_script():
    # Test data
    request_data = {
        "items": [{
            "isValid": True,
            "projectName": "Horror Test",
            "genre": "Horror",
            "storyIdea": "A person discovers their reflection has started acting independently",
            "duration": 1,
            "request_id": f"test-horror-{int(time.time())}"
        }]
    }
    
    print("=" * 70)
    print("HORROR SCRIPT GENERATION TEST")
    print("=" * 70)
...
if __name__ == "__main__":
    success = asyncio.run(test_horror_script())
